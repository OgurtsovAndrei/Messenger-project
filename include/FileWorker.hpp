...
struct bad_file_name_exception : file_exception {
    explicit bad_file_name_exception(const std::string &string)
        : file_exception(string){};
};

struct invalid_file_format_exception : file_exception {
    explicit invalid_file_format_exception(const std::string &string)
        : file_exception(string){};
};

struct read_file_exception : file_exception {
    explicit read_file_exception(const std::string &string)
        : file_exception(string){};
};
...
struct File {
    ...
    explicit File(nlohmann::json json_data, ParseJSON &) {
        if (!json_data["file"].is_string() ||
            !json_data["file_name"].is_string()) {
            throw invalid_file_format_exception(
                "Bad file format: in given json['file'] expected string!"
            );
        }
    ...
    explicit File(const std::string &file_path) {
        try {
            ...
            //            } catch (std::ios_base::failure &exception) { ENSURE:
            //            it should throws exactly this exception (sometimes it
            //            throes std::bad_alloc, if path is empty)
        } catch (std::exception &exception) {
            throw read_file_exception(
                "Unable to read the file: " + std::string(exception.what())
            );
        }
    ...
    [[nodiscard]] Status save(const std::string &file_save_directory) const {
        ...
        try {
            ...
            return Status(
                false, "Write file exception: " + std::string(exception.what())
            );
        }
        return Status(true, file_name);
    }

    [[nodiscard]] std::string get_name() const { return file_name; }
...
private:
    ...
    explicit File(const std::string &name, std::vector<uint8_t> data_)
        : file_name(remove_extra_file_name_head(name)),
          data(std::move(data_)) {}
