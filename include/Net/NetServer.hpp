            ...
            for (int consumer_id = 0; consumer_id < number_of_thread_in_pool; ++consumer_id) {
                consumers.push_back(std::move(std::thread([&, this]() mutable {
                    while (true) { // NOLINT
                        ...
                        std::cout << "Got request from user with --> " << connection_id << " connection id\n";
                        switch (request.request_type) {
                            ...
                            case FILE_DOWNLOAD_FAIL:
                                break;
                            case FILE_UPLOAD:
                                process_upload_file_request(user_connection, request);
                                break;
                            ...
                            case GET_USERS_IN_DIALOG_FAIL:
                                break;
                            case CHANGE_USER:
                                process_change_user_request(user_connection, request);
                                break;
    ...
    private:
        ...
        */
        ...
        void process_get_dialog_by_id_request(UserConnection &user_connection, DecryptedRequest request) {
            ...
            send_response_and_return_if_false(current_status.correct(), user_connection, GET_DIALOG_BY_ID_FAIL,
                                              "Get_dialog_by_id exception: " + current_status.message());
            user_connection.send_secured_request(DecryptedRequest(GET_DIALOG_BY_ID_SUCCESS, current_dialog));
        }
