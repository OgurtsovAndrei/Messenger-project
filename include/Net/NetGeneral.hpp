...
enum RequestType {
    ...
    GET_DIALOG_BY_ID,
    GET_DIALOG_BY_ID_SUCCESS,
    GET_DIALOG_BY_ID_FAIL,
    GET_USERS_IN_DIALOG,
    GET_USERS_IN_DIALOG_SUCCESS,
    GET_USERS_IN_DIALOG_FAIL,
    GET_ENCRYPTION,
    GET_ENCRYPTION_SUCCESS,
    GET_ENCRYPTION_FAIL,
    GET_ALL_ENCRYPTION,
    GET_ALL_ENCRYPTION_SUCCESS,
    GET_ALL_ENCRYPTION_FAIL,
    CHANGE_USER,
    CHANGE_USER_SUCCESS,
    CHANGE_USER_FAIL,
    SIGN_UP_REQUEST,
...
read_n_and_get_string(unsigned int n, boost::asio::ip::tcp::iostream &client) {
...
[[nodiscard]] inline std::string
convert_to_string_size_n(unsigned int value, unsigned int size = 16) {
...
struct DecryptedRequest {
    ...
    explicit DecryptedRequest(RequestType request_type_, json data_)
        : request_type(request_type_), data(std::move(data_)) {}

    explicit DecryptedRequest(RequestType request_type_)
        : request_type(request_type_) {}

    [[nodiscard]] EncryptedRequest encrypt(
        const Cryptographer::Encrypter &encrypter
    ) const;
...
struct EncryptedRequest {
    explicit EncryptedRequest(boost::asio::ip::tcp::iostream &connection) {
        std::string request_string_size =
            read_n_and_get_string(REQUEST_STRING_SIZE_IN_CHARS, connection);
        if (!is_number(request_string_size)) {
            std::cout << "<<" << request_string_size << ">>\n";
        }
        if (!is_number(request_string_size)) {
            throw std::runtime_error(
                "Bad connection! Expected request length, but received bytes "
                "are not number!"
            );
        }
    ...
    [[nodiscard]] DecryptedRequest decrypt(Cryptographer::Decrypter &decrypter
    ) const {
    ...
    [[nodiscard]] std::string to_text() const {
        ...
        text_request += convert_to_string_size_n(
            data_string.size(), REQUEST_STRING_SIZE_IN_CHARS
        );
...
private:
    ...
    explicit EncryptedRequest(std::string str_)
        : data_string(std::move(str_)) {}
};

inline EncryptedRequest DecryptedRequest::encrypt(
    const Cryptographer::Encrypter &encrypter
) const {
    json other = (*this);
...
inline EncryptedRequest DecryptedRequest::reinterpret_cast_to_encrypted(
) const {
    json other = (*this);
...
namespace {
inline void send_text_to_server(
