...
struct Message {
    ...
    explicit Message()
        : m_message_id(0), m_date_time(0), m_dialog_id(0), m_user_id(0) {}
    ...
    [[nodiscard]] std::string to_string() const {
        // ужасно не оптимально по трафику данные так гонять, но пока что
        // как есть, другого не дано. В качестве альтернативы вижу конвертацию в
        // reinterpret_cast<unsigned char*>, но тут возникают со стрингами
        // проблемы Если есть предложения, пишите.
        std::vector<std::string> message_data{std::to_string(m_message_id),
                                              std::to_string(m_date_time),
                                              m_text,
                                              m_file_name,
                                              std::to_string(m_dialog_id),
                                              std::to_string(m_user_id)};
        return convert_text_vector_to_text(message_data);
    }

    static Status parse_to_message(
        const std::string &text_data,
        Message &ref_to_answer_message
    ) {
        std::vector<std::string> message_data =
            convert_to_text_vector_from_text(text_data);
        ...
        if (!data_is_correct) {
            return Status(
                false, "Bad data: one of int expected fields is not int"
            );
        }
    ...
    static int callback_get_message_by_id(
        void *NotUsed,
        int argc,
        char **argv,
        char **azColName
    ...
    static int callback_get_message_list(
        void *NotUsed,
        int argc,
        char **argv,
        char **azColName
