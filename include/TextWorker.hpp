...
inline std::string
convert_to_string_size_n(unsigned int value, unsigned int size = 4) {
...
[[nodiscard]] inline std::string convert_int_vector_to_text(
    const std::vector<unsigned int> &int_vec
) {
    std::string answer;
    answer +=
        convert_to_string_size_n(int_vec.size(), INT_VECTOR_SIZE_SIZE_IN_CHARS);
...
[[nodiscard]] inline std::vector<unsigned int> convert_to_int_vector_from_text(
    const std::string &text
) {
    std::vector<unsigned int> answer;
    unsigned int vector_size =
        std::stoi(text.substr(0, INT_VECTOR_SIZE_SIZE_IN_CHARS));
    unsigned int covered_len = INT_VECTOR_SIZE_SIZE_IN_CHARS;
    for (auto index = 0; index < vector_size; ++index) {
        unsigned int element =
            std::stoi(text.substr(covered_len, INT_SIZE_SIZE_IN_CHARS));
...
[[nodiscard]] inline std::string convert_text_vector_to_text(
    const std::vector<std::string> &text_vec
) {
    std::string answer;
    answer += convert_to_string_size_n(
        text_vec.size(), TEXT_VECTOR_SIZE_SIZE_IN_CHARS
    );
    for (const std::string &element : text_vec) {
        answer += convert_to_string_size_n(
            element.size(), TEXT_VECTOR_ELEMENT_SIZE_IN_CHARS
        );
        answer += element;
    }
    return std::move(answer);
...
[[nodiscard]] inline std::vector<std::string> convert_to_text_vector_from_text(
    const std::string &text
) {
    std::vector<std::string> answer;
    unsigned int vector_size =
        std::stoi(text.substr(0, TEXT_VECTOR_SIZE_SIZE_IN_CHARS));
    unsigned int covered_len = TEXT_VECTOR_SIZE_SIZE_IN_CHARS;
    for (auto index = 0; index < vector_size; ++index) {
        unsigned int element_size = std::stoi(
            text.substr(covered_len, TEXT_VECTOR_ELEMENT_SIZE_IN_CHARS)
        );
