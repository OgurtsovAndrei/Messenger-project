...
convert_encrypted_data_to_text_vector(const EncryptedData &data_block) {
...
[[nodiscard]] inline EncryptedData make_encrypted_data_from_text_vector(
...
[[nodiscard]] inline EncryptedData make_encrypted_data_from_text(
    const std::string &text
) {
    return make_encrypted_data_from_text_vector(
        convert_to_text_vector_from_text(text)
    );
}

[[nodiscard]] inline std::string convert_encrypted_data_to_text(
    const EncryptedData &data_block
) {
    return convert_text_vector_to_text(
        convert_encrypted_data_to_text_vector(data_block)
    );
}

inline std::unique_ptr<Botan::Private_Key>
generate_keypair_RSA(const size_t bits, Botan::RandomNumberGenerator &rng) {
    return std::make_unique<Botan::RSA_PrivateKey>(rng, bits);
}

inline std::unique_ptr<Botan::Private_Key>
generate_keypair_DSA(const size_t bits, Botan::RandomNumberGenerator &rng) {
    return std::make_unique<Botan::ElGamal_PrivateKey>(
        rng, Botan::DL_Group("dsa/jce/1024")
    );
}

inline std::unique_ptr<Botan::Private_Key>
generate_keypair_DH(const size_t bits, Botan::RandomNumberGenerator &rng) {
    return std::make_unique<Botan::ElGamal_PrivateKey>(
        rng, Botan::DL_Group("dsa/jce/1024")
    );
}

inline EncryptedData encrypt(
    ...
    auto sym_cipher = Botan::AEAD_Mode::create_or_throw(
...
Botan::secure_vector<uint8_t> inline decrypt(
...
public:
    ...
    explicit Decrypter(Botan::AutoSeeded_RNG &rng_)
        : rng(rng_), key_pair(generate_keypair_DH(1024 /*  bits */, rng_)){};

    [[nodiscard]] std::string get_str_publicKey() const {
        return Botan::base64_encode(
            Botan::X509::BER_encode(*key_pair->public_key())
        );
    };
    ...
    [[nodiscard]] EncryptedData encrypt_text_to_encrypted_data_block(
        const std::string &plaintext
    ) const {
        return encrypt(
            as<Botan::secure_vector<uint8_t>>(plaintext), get_public_key(), rng
        );
    }

    [[nodiscard]] std::string encrypt_text_to_text(const std::string &plaintext
    ) const {
        return std::move(convert_encrypted_data_to_text(
            encrypt_text_to_encrypted_data_block(plaintext)
        ));
    }
...
public:
    ...
    explicit Encrypter(
        ...
        : rng(rng_) {
        Botan::SecureVector<uint8_t> publicKeyBytes(
            Botan::base64_decode(public_key_str)
        );
        public_key = std::unique_ptr<Botan::Public_Key>(Botan::X509::load_key(
            std::vector(publicKeyBytes.begin(), publicKeyBytes.end())
        ));
    };
